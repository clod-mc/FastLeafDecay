...
public final class FastLeafDecayPlugin extends JavaPlugin implements Listener {
  ...
  public void onEnable() {
    ...
    this.oneByOne = getConfig().getBoolean("OneByOne");
  ...
  private void onBlockRemove(final Block oldBlock, long delay) {
    ...
    if (!this.onlyInWorlds.isEmpty() && !this.onlyInWorlds.contains(worldName)) {
      return;
    }
    if (this.excludeWorlds.contains(worldName)) {
      return;
    }
    // No return
    ...
    for (BlockFace neighborFace : NEIGHBORS) {
      ...
      if (this.oneByOne) {
        ...
        getServer().getScheduler().runTaskLater(this, () -> this.decay(block), delay);
      }
      this.scheduledBlocks.add(block);
    }
  }
  ...
  private boolean decay(Block block) {
    ...
    if (this.playSound) {
      block
          .getWorld()
          .playSound(
              block.getLocation(), Sound.BLOCK_GRASS_BREAK, SoundCategory.BLOCKS, 0.05f, 1.2f);
    }
  ...
  private void decayOne() {
    ...
    do {
      ...
      Block block = this.scheduledBlocks.get(0);
      decayed = this.decay(block); // Will remove block from list.
    } while (!decayed);
    if (!this.scheduledBlocks.isEmpty()) {
